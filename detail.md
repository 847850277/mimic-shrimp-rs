...
- `POST /feishu/callback`
  - 接收飞书卡片回调，以及 `im.message.receive_v1` 的文本和语音消息事件。
  - 支持明文请求、`challenge` 校验回包、可选的 `encrypt` 解密。
  - 如果配置了 `FEISHU_CALLBACK_VERIFICATION_TOKEN`，会校验 payload 中的 token。
  - 文本消息会直接复用现有对话/学习链路；语音消息会先下载、转写，再复用相同链路生成回复。
  - 如果语音转写结果和最终回复都表现为英文，且已配置 TTS，会额外回送一条英语语音回复。
- `GET /tools`
...
- 每日英语学习能力
  ...
  - 同一个会话里会记住当天 lesson、重点句子和下一题进度，适合做连续学习而不是单轮问答。
  - 飞书里命中以下口令时，会优先走学习能力而不是普通聊天：
    - `开始今天的英语学习`
    - `这句话什么意思`
    - `再出一道题`
  - 如果当前会话已经进入当天学习上下文，用户直接发送一段英语跟读语音，也会尝试按重点句子做匹配和反馈。

## 飞书交互流程

1. 用户在飞书发送文本或语音消息到机器人。
2. 文本消息会先检查是否命中英语学习口令；语音消息会先下载飞书音频资源并转写成文本。
3. 如果当前会话已经进入当天英语学习，语音转写会优先按英文输入处理，尽量服务跟读场景。
4. 转写文本会先尝试命中学习口令，再尝试命中跟读评估；只有都不满足时，才回退到普通对话链路。
5. 如果命中跟读评估，系统会对照当前重点句子返回文本匹配度、命中关键词数量，以及建议补上或可能读错的词。
6. 文本结果会先通过飞书消息回复出去。
7. 如果用户原始语音像英文、系统最终文本回复也像英文，而且已经配置 `SPEECH_SYNTHESIS_*` 相关能力，系统会继续把这段英文回复合成为语音，并追加回送到同一条消息下。
8. 如果转写失败或语音长度异常，系统会直接回一条可操作的提示，让用户重新录制更清晰的语音。

## 交互式学习亮点

- 会话不是无状态的。开始今天的学习后，系统会记住当天 lesson、重点句子和当前题目进度，后续短口令就能继续推进。
- 同一条学习链路同时支持文本问答和语音跟读，不用在“聊天模式”和“练习模式”之间手动切换。
- 跟读反馈是围绕当天重点句子展开的，不是泛泛地返回一段聊天回复。
- 语音链路和 TTS 链路打通后，可以形成“用户说英文 -> 系统理解并用英文回答 -> 再附带一条英语音频”的闭环，适合做模仿和跟读。
- 当天卡片来自固定新闻源，内容每天都会更新，但交互方式保持稳定，用户容易形成持续练习习惯。
...
- `math_add`
- `time_now`
- `english_learning_start_today`（默认启用，可通过 `ENGLISH_LEARNING_ENABLED=false` 关闭）
  - 为当前会话启动“今日英语学习卡片”。
- `english_learning_explain_focus_sentence`（默认启用，可通过 `ENGLISH_LEARNING_ENABLED=false` 关闭）
  - 解释当前学习卡片里的重点句子。
- `english_learning_next_question`（默认启用，可通过 `ENGLISH_LEARNING_ENABLED=false` 关闭）
  - 返回当前学习卡片里的下一道理解题。
- `english_learning_shadowing_feedback`（默认启用，可通过 `ENGLISH_LEARNING_ENABLED=false` 关闭）
  - 对当前会话的英语跟读文本做评估；参数中需要提供 `transcript`。
